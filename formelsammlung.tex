...
Rechteck (\textbf{kausal}, nicht zentriert!):
\[ \operatorname{rect}\!\big(\tfrac{t}{T}\big)=\begin{cases}1&0\le t<T\\0&\text{sonst}\end{cases},\quad \text{Fläche}=T \]
$\operatorname{rect}\!\big(\tfrac{t-a}{b}\big)=1$ auf $[a,\,a+b)$ — Argument nennt den \textbf{linken Rand}.
\[ \varepsilon(t)-\varepsilon(t-T)=\operatorname{rect}\!\big(\tfrac{t}{T}\big),\quad
   \varepsilon(t-a)-\varepsilon(t-b)=\operatorname{rect}\!\big(\tfrac{t-a}{b-a}\big) \]
...
Impulsantwort $h=\mathcal S[\delta]$, System vorher in Ruhe ($\vec z=0$). Faltung:
...
Sprungantwort $g=\mathcal S[\varepsilon]=\varepsilon*h$:
\[ g(t)=\!\int_{-\infty}^{t}\! h(\tau)\,\mathrm d\tau,\qquad h(t)=\tfrac{\mathrm dg(t)}{\mathrm dt} \]
\[ g[k]=\sum_{l=-\infty}^{k} h[l],\qquad h[k]=g[k]-g[k-1] \]
Reihe: $h_{\text{ges}}=h_1*h_2$;\quad parallel: $h_{\text{ges}}=h_1+h_2$.\\
Spiegeln: $h(t_0-t)\ne0$ auf $(t_0-T,\,t_0]$, falls $h\ne0$ auf $[0,T)$.\\
...
IIR 1.\,Ordn.: $y[k]=b_0x[k]+a_1y[k{-}1]\Rightarrow h[k]=b_0\,a_1^{\,k}\,\varepsilon[k]$,\\
BIBO-stabil $\Leftrightarrow|a_1|<1$.\quad Rezept: $x=\delta$, $y[k{<}0]=0$, Werte, Muster.\\
\emph{Vorzeichen von $a_\mu$ aus der Angabe bzw.\ dem Blockdiagramm ablesen} --
Skript und Blockdiagramme nutzen unterschiedliche Konventionen.
