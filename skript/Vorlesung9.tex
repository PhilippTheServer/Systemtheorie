...
\begin{definition}[Wahrscheinlichkeitsverteilung]
...
\begin{definition}[Erwartungswert, quadratischer Mittelwert und Varianz]
...
Diese Kenngrößen sind die theoretischen Gegenstücke zu den empirischen Lage- und Streumaßen (Abschnitt~\ref{sec:streumasse}).
...
\begin{definition}[Gleichverteilung auf einem Intervall]
...
\begin{beweis}[Momente der Exponentialverteilung]
...
\begin{definition}[Laplaceverteilung]
...
\begin{beweis}[Momente der Laplaceverteilung]
...
Die Abtastwerte eines Audiosignals seien Laplace-verteilt mit $\mu = 0$ und $\sigma = 1$. Die Abtastwerte werden nun mit der nachfolgend dargestellten Quantisierungskennlinie (vgl.\ Abschnitt~\ref{sec:quantisierungskennlinie}) quantisiert.
...
\begin{bemerkung}[Übersicht: Momente der stetigen Verteilungen]
\renewcommand{\arraystretch}{1.6}
\begin{center}
\begin{tabular}{lccc}
  \hline
  \textbf{Verteilung} & \textbf{Dichte $p_X(x)$} & $\mathbf{E\{X\}}$ & $\mathbf{var\{X\}}$ \\
  \hline
  Gleichverteilung $[a,b]$ & $\dfrac{1}{b-a}$ auf $[a,b]$ & $\dfrac{a+b}{2}$ & $\dfrac{(b-a)^2}{12}$ \\
  Normalverteilung & $\dfrac{1}{\sqrt{2\pi\sigma^2}}\,e^{-\frac{(x-\mu)^2}{2\sigma^2}}$ & $\mu$ & $\sigma^2$ \\
  Exponentialverteilung & $\lambda e^{-\lambda x}$,\ $x \geq 0$ & $\dfrac{1}{\lambda}$ & $\dfrac{1}{\lambda^2}$ \\
  Laplaceverteilung & $\dfrac{1}{2\sigma}\,e^{-\frac{|x-\mu|}{\sigma}}$ & $\mu$ & $2\sigma^2$ \\
  \hline
\end{tabular}
\end{center}
...
\begin{satz}[Transformation einer streng monotonen Funktion]
...
\begin{satz}[Transformation einer nichtmonotonen Funktion]
