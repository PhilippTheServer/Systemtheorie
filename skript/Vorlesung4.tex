...
\label{chap:abtastung-quantisierung}

\section{Analog-digital Umsetzung}
\label{sec:analog-digital}
...
Die Abtastung eines analogen Signals führt zu einer Entnahme von ,,Signalproben'' in regelmäßigen Intervallen. In digitalen Telefonsystemen (ISDN, GSM, etc.) werden dem analogen Sprachsignal z.\,B.\ 8000 Abtastwerte pro Sekunde entnommen.

\begin{definition}[Abtastrate]
Die \textit{Abtastfrequenz} oder \textit{Abtastrate} $f_A$ ist definiert als
...
\begin{itemize}
  ...
  $$
  mit $x[k] = x(kT_A)$ erfüllt ist. Die Funktionen $g_k(t)$ können z.\,B.\ mit Hilfe der Fouriertransformation (Vorlesung Systemtheorie~II) oder der Lagrange-Polynome gefunden werden. Es handelt sich dabei um die si-Funktion (Abschnitt~\ref{sec:elementare-analoge-signale}),
  $$
...
\begin{beweis}[Ideale Rekonstruktion über Lagrange-Polynome]
...
\begin{definition}[Quantisierung und Quantisierungsfehler]
...
\label{sec:quantisierungskennlinie}
...
\label{sec:quantisierungsfehler}
...
\begin{beweis}[Leistung des Quantisierungsfehlers]
...
das außerhalb des betrachteten Intervalls $[-T, T]$ periodisch fortgesetzt werden kann. Dieses Signal dient als Modell für ein gleichverteiltes Signal (vgl.\ Abschnitt~\ref{sec:gleichverteilung}), in dem alle Amplituden $x(t)$ mit gleicher Häufigkeit auftreten. Mit der Substitution $u = t/T$ ergibt sich die Signalleistung $P_x$ zu:
