...
Summe und Produkt werden später erneut wichtig, wenn Faltung und Korrellation behandelt werden.
...
Ein Zeitdiskretes Signal $x$ wird als Folge von Abstastwerten
$x[k]$ mit $k \in \mathbb{Z}$ 
...
definiert. Falls eine physikalische Interpretation des Signals gewünscht ist,
muss die Dauer des Abtastintervalls bzw. deren Kehrwert, die Abtastrate
$f_A = \frac{1}{T_A}$, bekannt sein.

% ===========================================
% Image of Graph is missing here
% ===========================================

\section{Schwebung}

Eine Schwebung entsteht durch die Addition zweier Sinussignale mit gleicher 
Amplitude und geringem Frequenzunterschied $2 \delta \omega$:

$$
\begin{aligned}
y(t) &= A e^{j(\omega_0+\Delta\omega)t} + A e^{j(\omega_0-\Delta\omega)t} \\
     &= A e^{j\omega_0 t} \left( e^{j\Delta\omega t} + e^{-j\Delta\omega t} \right) \\
     &= 2A \cos(\Delta\omega\, t)\, e^{j\omega_0 t}
\end{aligned}
$$

wobei $\omega_0 = 2 \pi f_0$ die mittlere Frequenz angibt. Die Real- und Imaginärteile sind:

$$
\begin{array}{rl}
Re\{y(t)\} = & 2A cos(\delta \omega t) cos(\omega_0 t)\\
Im\{y(t)\} = & 2A cos(\delta \omega t) sin(\omega_0 t)
\end{array}
$$


Die Schwebung äußert sich in einer Modulation durch die nun zeitvariante
Amplitude $2A cos(\delta \omega t)$. Dieser (zunächst zeitkontinuierlich motivierte) Effekt
wird nun wie folgt zeitdiskret implementiert:
$x1[k] = sin((\omega_0 - \delta \omega)k T_A)$
$x2[k] = sin((\omega_0 + \delta \omega)k T_A)$
und dem Summensignal $z(k T_A) = \frac{1}{2} (x_1[k] + _x2[k]) = \frac{1}{2} Im\{y(k T_A)\}$.
