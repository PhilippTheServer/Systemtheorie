...
und Phasensignal formuliert werden. Dabei bezeichnet $j = \sqrt{-1}$ die imaginäre Einheit.

$$
\begin{array}{rl}
\underline{x}(t) = x_R(t) + jx_I(t) = & \operatorname{Re}\{\underline{x}(t)\} + j\operatorname{Im}\{\underline{x}(t)\}\\
= & |\underline{x}(t)|\,e^{j \phi(t)}
\end{array}
$$

Mit der Definition des zu $\underline{x}(t)$ konjugierten Signals $\underline{x}^*(t) = x_R(t) - jx_I(t)$ gilt:

$$
\begin{array}{rl}
x_R(t) = & \operatorname{Re}\{\underline{x}(t)\} = \dfrac{1}{2}\bigl(\underline{x}(t) + \underline{x}^*(t)\bigr) \\[8pt]
x_I(t) = & \operatorname{Im}\{\underline{x}(t)\} = \dfrac{1}{2j}\bigl(\underline{x}(t) - \underline{x}^*(t)\bigr) \\[8pt]
|\underline{x}(t)| = & \sqrt{x_R^2(t) + x_I^2(t)} = \sqrt{\underline{x}(t)\cdot \underline{x}^*(t)} \\[8pt]
\phi(t) = & \arg\{\underline{x}(t)\}
\end{array}
$$
...
Das Gleichsignal ist durch $x(t) = A\ \forall t$ definiert. Hierbei ist $A$ eine Konstante mit $A \in \mathbb{R}$ oder $A \in \mathbb{C}$; $T$ bezeichnet ein zeitliches Bezugsintervall.
...
Den sinus- oder cosinusförmigen Signalen kommt eine besondere Bedeutung zu, da viele natürliche Signale harmonische Strukturen aufweisen und die Analyse linearer Systeme auf ihrer Grundlage besonders einfach ist. Das Sinussignal ist wie folgt definiert:

  $$
  x(t) = A \sin(\omega t+ \phi) = A \cos\!\left(\omega t + \phi - \tfrac{\pi}{2}\right)
  $$
...
Ein weiteres, interessantes Signal ist durch die si-Funktion gegeben. Diese ist eine gerade symmetrische Funktion und für $-\infty < t < \infty$ wie folgt definiert:
...
Die Definition der si-Funktion beruht auf einer stetigen Ergänzung der Funktion $\frac{\sin(\pi t/T)}{\pi t/T}$ an der Stelle $t = 0$.
...
Im Zusammenhang mit der Theorie linearer Systeme (siehe Abschnitt~\ref{sec:zeitinvarianz-linearitaet})
und Netzwerken im eingeschwungenen Zustand wird häufig auch das komplexe Exponentialsignal verwendet,
...
Der Dirac-Impuls ist ein Signal, das für die mathematische Behandlung der Abtastung analoger Signale (Kapitel~\ref{chap:abtastung-quantisierung}) von großer Bedeutung ist. Den Dirac-Impuls erhält man im Grenzwert $T \to 0$ aus Rechteckfunktionen der Form $x(t) = \frac{1}{T}\operatorname{rect}\!\bigl(\frac{t + T/2}{T}\bigr)$.
...
\begin{bemerkung}[Der Dirac-Impuls als Distribution]
In dem Katalog der bisher behandelten Elementarsignale nimmt der Dirac-Impuls eine Sonderstellung ein, da er nicht der Kategorie der Funktionen zuzuordnen ist. Es handelt sich hierbei um eine sogenannte \textit{Distribution}. Daher sind einerseits die Methoden der Standardanalysis nicht ohne weiteres anwendbar. Zum Beispiel ist die Frage nach der „Energie" oder der „Leistung" des Dirac-Impulses nicht sinnvoll zu beantworten (zu Energie und Leistung siehe Abschnitt~\ref{sec:norm-energie-leistung}). Andererseits erweitert der Dirac-Impuls den mathematischen Werkzeugkasten und ist u.\,a.\ im Zusammenhang mit der Fourier-Transformation (Kapitel~\ref{chap:signalsynthese}) von Bedeutung.
\end{bemerkung}
...
\begin{definition}[Diskrete Einheitsimpulsfolge]
...
\begin{definition}[Diskreter Einheitssprung]
...
\begin{definition}[Diskretes Rechtecksignal]
...
\begin{definition}[Diskrete Exponentialfolge]
...
\begin{definition}[Diskretes Sinussignal]
