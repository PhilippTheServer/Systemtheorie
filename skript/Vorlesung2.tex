...
konjugiert komplexe Signal bezeichnet (Abschnitt~\ref{sec:komplexwertige-signale}).
...
Einige wichtige Elementarsignale (Sprungfunktion, Sinussignal; Abschnitt~\ref{sec:elementare-analoge-signale}) sind
...
Offensichtlich muss die Autokorrelationsfunktion $\varphi_{xx}$ bei $\tau = 0$ bzw. $\varkappa = 0$ maximal werden.

\begin{beweis}[Die AKF reellwertiger Signale ist gerade symmetrisch]
Wir zeigen nun am Beispiel eines analogen, reellwertigen Signals, dass die AKF eine gerade symmetrische Funktion ist. Ausgehend von der Definition und der Substitution $t = u - \tau$ erhalten wir:
...
\begin{definition}[Orthogonal- und Orthonormalsystem]
...
\begin{definition}[Signalbandbreite]
...
\begin{definition}[Diskrete Fourier-Transformation (DFT)]
...
Wenn das diskrete Zeitsignal durch Abtastung (Abschnitt~\ref{sec:abtastung}) mit der Abtastperiode $T_A$ aus einem analogen Signal hervorgegangen ist, gilt für die physikalische Frequenz des $m$-ten Koeffizienten mit $m < M/2$ die Beziehung
